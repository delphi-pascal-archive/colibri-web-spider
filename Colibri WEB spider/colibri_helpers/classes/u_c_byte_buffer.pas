...
unit u_c_byte_buffer;
  interface
    ...
    type c_byte_buffer= class(c_basic_object)
                          ...
                          function f_display_read_write_index: String;
                          ...
                          procedure skip_blanks(var pv_index: Integer);
                          function f_extract_non_blank(var pv_index: Integer): String;
                          function f_extract_eol(var pv_index: Integer): String;
                          ...
                          procedure save_to_file_start_end(p_full_file_name: String;
                              p_start_index, p_end_index: Integer);
  ...
  implementation
    ...
    function c_byte_buffer.f_display_read_write_index: String;
      begin
        Result:= Format('r=%4d w=%4d', [m_read_index, m_write_index]);
      end; // f_display_read_write_index
    ...
    function c_byte_buffer.f_string_position(p_index: Integer; p_string: String): Integer;
      ...
      var l_length: Integer;
          ...
          l_string: String;
      begin
        // display('start_at '+ IntToStr(p_index));
        l_length:= Length(p_string);
        if l_length<= 0
          then begin
              if p_index= 0
        ...
        // display('not_found');
        // -- did not find.
        // -- Result in 1 position too far => get back
        Dec(Result);
        if Result< p_index
          then Result:= p_index;
    ...
    procedure c_byte_buffer.skip_blanks(var pv_index: Integer);
      begin
        while (pv_index< m_write_index)
          and (chr(m_oa_byte_buffer[pv_index]) in k_blanks) do
            Inc(pv_index);
      end; // skip_blanks

    function c_byte_buffer.f_extract_non_blank(var pv_index: Integer): String;
      var l_start_index: Integer;
      begin
        l_start_index:= pv_index;
        while (pv_index<= m_write_index)
            and not (chr(m_oa_byte_buffer[pv_index]) in k_blanks) do
          Inc(pv_index);

        Result:= f_extract_string_start_end(l_start_index, pv_index- 1);
      end; // f_extract_non_blank

    function c_byte_buffer.f_extract_eol(var pv_index: Integer): String;
      var l_start_index: Integer;
      begin
        l_start_index:= pv_index;
        if f_skip_return_line_feed(pv_index)
          then Result:= f_extract_string_start_end(l_start_index, pv_index- 3)
          else Result:= '';
      end; // f_extract_eol
...
*)
            ...
            then
              if f_search_rest(pv_index)
                then begin
                    ...
                    Result:= pv_index;
                    Inc(pv_index, Length(p_search_string)- 1);
                    exit;
                  end;
    ...
    procedure c_byte_buffer.save_to_file_start_end(p_full_file_name: String;
        p_start_index, p_end_index: Integer);
      var l_file: File;
      begin
        AssignFile(l_file, p_full_file_name);
        ...
        BlockWrite(l_file, m_oa_byte_buffer[p_start_index], p_end_index+ 1- p_start_index);

        CloseFile(l_file);
      end; // save_to_file_start_end
